...
app.use(async (ctx, next) => {
  ...
  await new Promise(resolve =>
    reader.on('close', () => {
      ...
      const getLabel = fileName => `python -m scripts.label_image --graph=tf_files/retrained_graph.pb --image=${fileName}`

      exec(getLabel(fileName))
        .then(({ stdout, stderr }) => stdout)
        .then(R.pipe(R.split('\n'), R.filter(Boolean)))
        .then(([time, ...labels]) =>
          labels.map(R.split(' '))
            .map(([name, probability]) => [name, Number(probability).toFixed(2)]))
        .then(labels => {
          console.log(labels)
          const [flower, probability] = labels[0]
          ctx.body = JSON.stringify([labels[0], labels[1]])
          const paths = fileName.split('/')
          const newFile = `${flower}.${probability}.${paths.pop()}`

          fs.rename(fileName, path.join(paths.join('/'), newFile), console.error)
